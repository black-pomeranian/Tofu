...
public class Bluetooth : MonoBehaviour
{
    ...
    const string CHARACTERISTIC_ATTITUDE = "10B20106-5B3B-4571-9508-CF3EFCD7BBAE";
        ...
        if (_timeout > 0f)
        {
            ...
            if (_timeout <= 0f)
            {
                ...
                if (state == State.Control)
                {
                    ...
                    byte[] buff = { 2, 1, 1, 100, 2, 1, 100, 100 };
                    // モーターcharacteristicに対してパケット送信
                    //characteristicTable[CHARACTERISTIC_MOTOR].WriteValue(data: buff, withResponse: false);
                    characteristicTable[CHARACTERISTIC_ATTITUDE].WriteValue(data: buff, withResponse: false);
                    // 送信処理を50ミリ秒間隔で実行
